...
##fungi
core_fungi_NSCLC<-read.csv("7.featureseletions/logistic_response/fungi/NSCLC/Q3/Logistic_NSCLC_filter_fungi_meta_pvalue.csv",header = T,check.names = F)
core_fungi_NSCLC$id<-gsub("_"," ",core_fungi_NSCLC$id)
core_fungi_NSCLC<-subset(core_fungi_NSCLC,core_fungi_NSCLC$pvalue_meta<0.05&core_fungi_NSCLC$het_p>0.05)
fungi_rela<-read.csv("5.fungi_data/NSCLC/relabundance/species/MMUPHIN/fungi_NSCLC_whole_rela_filter_MMUPHIN.csv",header = T,check.names = F,row.names = 1)
rownames(fungi_rela)<-gsub("_"," ",rownames(fungi_rela))
core_fungi_rela<-fungi_rela[core_fungi_NSCLC$id,]
core_fungi_rela$type<-"Fungi"
core_fungi_rela$color<-"#A7BE68"

##bacteria
core_bac_NSCLC<-read.csv("7.featureseletions/logistic_response/bacteria/NSCLC/Q3/Logistic_NSCLC_meta_filter_bac_pvalue.csv",header = T,check.names = F)
core_bac_NSCLC$id<-gsub("_"," ",core_bac_NSCLC$id)
core_bac_NSCLC<-subset(core_bac_NSCLC,core_bac_NSCLC$pvalue_meta<0.05&core_bac_NSCLC$het_p>0.05)
bac_rela<-read.csv("4.bacteria_data/NSCLC/relabundance/species/MMUPHIN/bac_NSCLC_whole_rela_filter_MMUPHIN.csv",header = T,check.names = F,row.names = 1)
rownames(bac_rela)<-gsub("_"," ",rownames(bac_rela))
core_bac_rela<-bac_rela[core_bac_NSCLC$id,]
core_bac_rela$type<-"Bacteria"
core_bac_rela$color<-"#EEB977"

##metadata
meta_NSCLC<-read.csv("2.rawData/clinical/final/filter/cli_NSCLC_filter.csv",header = T,row.names = 1)
meta_NSCLC<-subset(meta_NSCLC,!is.na(response_code))
NSCLC_fungi_bac<-rbind(core_fungi_rela,core_bac_rela)
table(meta_NSCLC$dataset)
NSCLC_fungi_bac_rela<-NSCLC_fungi_bac[,rownames(meta_NSCLC)]
rownames(NSCLC_fungi_bac_rela)<-gsub("\\[|\\]", "",rownames(NSCLC_fungi_bac_rela))
write.csv(NSCLC_fungi_bac_rela,"8.correlation/NSCLC/Q3/NSCLC_fungi_bac_core_rela_Q3.csv")

rownames(NSCLC_fungi_bac)<-gsub("\\[|\\]", "",rownames(NSCLC_fungi_bac))
NSCLC_fungi_bac$id<-rownames(NSCLC_fungi_bac)
NSCLC_fungi_bac$Label<-sapply(NSCLC_fungi_bac$id, abbreviate_microbial_name)
node<-NSCLC_fungi_bac[,c(ncol(NSCLC_fungi_bac)-1,ncol(NSCLC_fungi_bac)-3,ncol(NSCLC_fungi_bac)-2,ncol(NSCLC_fungi_bac))]
write.csv(node,"8.correlation/NSCLC/Q3/NSCLC_fungi_bac_node_Q3.csv",row.names = F)
...
##不同组别间相关性系数差异分析
response_design_mat <- lapply(meta_NSCLC[colnames(NSCLC_fungi_bac_rela), 'response_code'], function(x){
  if (x == "1") {
    return(c(1,0))
  }else{
    return(c(0,1))
...
##spearman                   
set.seed(123)
ddcor_res <- ddcorAll(inputMat = NSCLC_fungi_bac_rela, 
                      corrType = 'spearman', 
...
write.csv(all_dgca,"8.correlation/NSCLC/Q3/DCGA_spearman_NSCLC_Q3.csv",row.names=F)
...
write.csv(all_dgca_R,"8.correlation/NSCLC/Q3/DCGA_spearman_NSCLC_R_Q3.csv",row.names=F)
write.csv(all_dgca_NR,"8.correlation/NSCLC/Q3/DCGA_spearman_NSCLC_NR_Q3.csv",row.names=F)

write.csv(node_R,"8.correlation/NSCLC/Q3/DCGA_spearman_NSCLC_R_node_Q3.csv",row.names=F)
write.csv(node_NR,"8.correlation/NSCLC/Q3/DCGA_spearman_NSCLC_NR__node_Q3.csv",row.names=F)

##pearson
set.seed(123)
ddcor_res_pearson <- ddcorAll(inputMat = NSCLC_fungi_bac_rela, 
                              corrType = 'pearson', 
                              design = response_design_mat,#design必须是数值型的
                              nPerms = 20,
                              compare = c("Response","Non-response"))
...
write.csv(all_dgca_pearson,"8.correlation/NSCLC/Q3/DCGA_pearson_NSCLC_Q3.csv",row.names=F)
...
write.csv(all_dgca_pearson_R,"8.correlation/NSCLC/Q3/DCGA_pearson_NSCLC_R_Q3.csv",row.names=F)
write.csv(all_dgca_pearson_NR,"8.correlation/NSCLC/Q3/DCGA_pearson_NSCLC_NR_Q3.csv",row.names=F)

write.csv(node_pearson_R,"8.correlation/NSCLC/Q3/DCGA_pearson_NSCLC_R_node_Q3.csv",row.names=F)
write.csv(node_pearson_NR,"8.correlation/NSCLC/Q3/DCGA_pearson_NSCLC_NR__node_Q3.csv",row.names=F)
