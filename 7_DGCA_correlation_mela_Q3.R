...
##fungi
core_fungi_mela<-read.csv("7.featureseletions/logistic_response/fungi/melanoma/Q3/Logistic_melanoma_all_fungi_meta_pvalue.csv",header = T,check.names = F)
core_fungi_mela$id<-gsub("_"," ",core_fungi_mela$id)
core_fungi_mela<-subset(core_fungi_mela,core_fungi_mela$pvalue_meta<0.05&core_fungi_mela$het_p>0.05)
fungi_rela<-read.csv("5.fungi_data/melanoma/relabundance/species/MMUPHIN/fungi_mela_whole_rela_filter_MMUPHIN.csv",header = T,check.names = F,row.names = 1)
rownames(fungi_rela)<-gsub("_"," ",rownames(fungi_rela))
core_fungi_rela<-fungi_rela[core_fungi_mela$id,]
core_fungi_rela$type<-"Fungi"
core_fungi_rela$color<-"#A7BE68"

##bacteria
core_bac_mela<-read.csv("7.featureseletions/logistic_response/bacteria/melanoma/Q3/Logistic_melanoma_meta_filter_bac_pvalue.csv",header = T,check.names = F)
core_bac_mela$id<-gsub("_"," ",core_bac_mela$id)
core_bac_mela<-subset(core_bac_mela,core_bac_mela$pvalue_meta<0.05&core_bac_mela$het_p>0.05)
bac_rela<-read.csv("4.bacteria_data/melanoma/relabundance/species/MMUPHIN/bac_mela_whole_rela_filter_MMUPHIN.csv",header = T,check.names = F,row.names = 1)
rownames(bac_rela)<-gsub("_"," ",rownames(bac_rela))
core_bac_rela<-bac_rela[core_bac_mela$id,]
core_bac_rela$type<-"Bacteria"
core_bac_rela$color<-"#EEB977"

##metadata
meta_mela<-read.csv("2.rawData/clinical/final/filter/cli_mela_filter.csv",header = T,row.names = 1)
mela_fungi_bac<-rbind(core_fungi_rela,core_bac_rela)
table(meta_mela$dataset)
meta_mela_sub<-subset(meta_mela,meta_mela$dataset!="PRJNA541981")
mela_fungi_bac_rela<-mela_fungi_bac[,rownames(meta_mela_sub)]
rownames(mela_fungi_bac_rela)<-gsub("\\[|\\]", "",rownames(mela_fungi_bac_rela))
write.csv(mela_fungi_bac_rela,"8.correlation/melanoma/Q3/mela_fungi_bac_core_rela_Q3.csv")

rownames(mela_fungi_bac)<-gsub("\\[|\\]", "",rownames(mela_fungi_bac))
mela_fungi_bac$id<-rownames(mela_fungi_bac)
mela_fungi_bac$Label<-sapply(mela_fungi_bac$id, abbreviate_microbial_name)
node<-mela_fungi_bac[,c(ncol(mela_fungi_bac)-1,ncol(mela_fungi_bac)-3,ncol(mela_fungi_bac)-2,ncol(mela_fungi_bac))]
write.csv(node,"8.correlation/melanoma/Q3/mela_fungi_bac_node_Q3.csv",row.names = F)
...
##不同组别间相关性系数差异分析
response_design_mat <- lapply(meta_mela_sub[colnames(mela_fungi_bac_rela), 'response_code'], function(x){
  if (x == "1") {
    return(c(1,0))
  }else{
    return(c(0,1))
...
##spearman                   
set.seed(123)
ddcor_res <- ddcorAll(inputMat = mela_fungi_bac_rela, 
                      corrType = 'spearman', 
...
write.csv(all_dgca,"8.correlation/melanoma/Q3/DCGA_spearman_mela_Q3.csv",row.names=F)
...
all_dgca_R$color<-ifelse(all_dgca_R$sig=="P","#BE68A7","#428dbf")
...
all_dgca_NR$color<-ifelse(all_dgca_NR$sig=="P","#BE68A7","#428dbf")
...
write.csv(all_dgca_R,"8.correlation/melanoma/Q3/DCGA_spearman_mela_R_Q3.csv",row.names=F)
write.csv(all_dgca_NR,"8.correlation/melanoma/Q3/DCGA_spearman_mela_NR_Q3.csv",row.names=F)

write.csv(node_R,"8.correlation/melanoma/Q3/DCGA_spearman_mela_R_node_Q3.csv",row.names=F)
write.csv(node_NR,"8.correlation/melanoma/Q3/DCGA_spearman_mela_NR__node_Q3.csv",row.names=F)

##pearson####
set.seed(123)
ddcor_res_pearson <- ddcorAll(inputMat = mela_fungi_bac_rela, 
                              corrType = 'pearson', 
                              design = response_design_mat,#design必须是数值型的
                              nPerms = 10,
                              compare = c("Response","Non-response"))
...
all_dgca_pearson$Response_pVal_adj <-p.adjust(all_dgca_pearson$Response_pVal, method = "BH")
all_dgca_pearson$'Non-response_pVal_adj' <-p.adjust(all_dgca_pearson$`Non-response_pVal`, method = "BH")
...
write.csv(all_dgca_pearson,"8.correlation/melanoma/Q3/DCGA_pearson_mela_Q3.csv",row.names=F)
...
all_dgca_pearson_R$color<-ifelse(all_dgca_pearson_R$sig=="P","#BE68A7","#428dbf")
...
all_dgca_pearson_NR$color<-ifelse(all_dgca_pearson_NR$sig=="P","#BE68A7","#428dbf")
...
write.csv(all_dgca_pearson_R,"8.correlation/melanoma/Q3/DCGA_pearson_mela_R_Q3.csv",row.names=F)
write.csv(all_dgca_pearson_NR,"8.correlation/melanoma/Q3/DCGA_pearson_mela_NR_Q3.csv",row.names=F)

write.csv(node_pearson_R,"8.correlation/melanoma/Q3/DCGA_pearson_mela_R_node_Q3.csv",row.names=F)
write.csv(node_pearson_NR,"8.correlation/melanoma/Q3/DCGA_pearson_mela_NR__node_Q3.csv",row.names=F)
